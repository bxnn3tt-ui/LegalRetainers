    ...
    try {
      if (isCaseOrder) {
        ...
        const payload = {
          ...
          requirements: caseOrderData.requirements || undefined,
          yearsInPractice: caseOrderData.yearsInPractice,
          referralSource: caseOrderData.referralSource,
          serviceAgreement: caseOrderData.serviceAgreement
        };

        const response = await fetch('/api/send-claim-order', {
          method: 'POST',
          headers: {
        ...
        let errorMessage = 'There was an error submitting your case request. Please try again.';
        if (!response.ok) {
          try {
      ...
      const partnershipPayload = {
        ...
        challenge: [
          partnershipData.partnershipGoals && `Partnership goals: ${partnershipData.partnershipGoals}`,
          partnershipData.currentChallenges && `Current challenges: ${partnershipData.currentChallenges}`,
          partnershipData.additionalInfo && `Additional info: ${partnershipData.additionalInfo}`
        ].filter(Boolean).join('\n').slice(0, 500)
      };

      const partnershipResponse = await fetch('/api/send-law-firm-lead', {
        method: 'POST',
        headers: {
          'Content-Type': 'application/json'
        },
      ...
      let partnershipError = 'There was an error submitting your inquiry. Please try again.';
      if (!partnershipResponse.ok) {
        try {
          const errorData = await partnershipResponse.json();
          if (errorData?.error) partnershipError = errorData.error;
        } catch {
          // Keep default error message when response body is not JSON.
        }
        setFormError(partnershipError);
        return;
      }
      ...
      setFormError('There was an error submitting your request. Please check your connection and try again.');
    } finally {
              ...
              <div className="flex justify-between pt-6">
                ...
                <div className="ml-auto">
                  {step < totalSteps ? (
                    ...
                    <Button onClick={handleSubmit} variant="brutalist" className="lr-focus" disabled={isSubmitting}>
